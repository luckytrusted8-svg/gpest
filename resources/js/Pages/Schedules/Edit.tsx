...
import { Clock, MapPin, Sparkles, Building2, UserCheck, ShieldCheck, AlertCircle, CheckCircle2, Star, PenLine } from 'lucide-react';
...
export default function Edit({ schedule, customers = [], contracts = [], technicians = [], supervisors = [] }: Props) {
    const isInitial24h = (schedule.jam_mulai?.startsWith('00:00') && schedule.jam_selesai?.startsWith('23:59'));
    const [is24HoursFlexible, setIs24HoursFlexible] = useState(Boolean(isInitial24h));
    const [locationSelection, setLocationSelection] = useState<string>('custom');
    const [isCustomLocation, setIsCustomLocation] = useState(false);

    const { data, setData, put, processing, errors } = useForm<FormData>({
        schedule_code: schedule.schedule_code || '',
        customer_id: String(schedule.customer_id) || '',
        contract_id: schedule.contract_id ? String(schedule.contract_id) : '',
        lokasi: schedule.lokasi || '',
        jenis_layanan: schedule.jenis_layanan || 'General Pest Control',
        technician_id: schedule.technician_id ? String(schedule.technician_id) : '',
        supervisor_id: schedule.supervisor_id ? String(schedule.supervisor_id) : '',
        tanggal: schedule.tanggal || '',
        jam_mulai: schedule.jam_mulai ? schedule.jam_mulai.substring(0, 5) : '08:00',
        jam_selesai: schedule.jam_selesai ? schedule.jam_selesai.substring(0, 5) : '17:00',
        prioritas: schedule.prioritas || 'normal',
    ...
    const handleCustomerChange = (val: string) => {
        ...
        setData((prev) => ({
            ...
            customer_id: val,
            contract_id: '',
            lokasi: autoAddress || prev.lokasi,
        }));
    ...
    return (
        <AppLayout>
            <Head title={`Edit Jadwal ${schedule.schedule_code} - G-PEST`} />

            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-slate-200">
                    <div>
                        <h1 className="text-xl font-bold text-slate-900">Edit Jadwal Pekerjaan</h1>
                        <p className="text-xs text-slate-500 mt-0.5">Perbarui penugasan, lokasi, atau status jadwal teknisi.</p>
                    </div>
                ...
                <div className="bg-white border border-slate-200/90 rounded-2xl shadow-sm p-6 sm:p-8">
                    <form onSubmit={submit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* Lokasi Pekerjaan (Dropdown Rapi: Lokasi Utama vs Cabang) */}
                            <div className="space-y-1.5">
                                <div className="flex items-center justify-between">
                                    <Label htmlFor="lokasi_select" className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                                        <MapPin className="w-3.5 h-3.5 text-slate-500" />
                                        <span>Lokasi Pekerjaan</span>
                                    </Label>
                                    <span className="text-[11px] text-slate-400 font-medium">Pilih dari database atau tulis manual</span>
                                </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                {selectedTech && detectedJobArea && !isAreaMatch(selectedTech.technician?.area_tugas, detectedJobArea) && (
                                    <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-900 flex items-start gap-2 mt-1.5">
                                        ...
                                        <div>
                                            <span className="font-semibold">Catatan Perbedaan Wilayah:</span> Teknisi <strong>{selectedTech.name}</strong> memiliki area tugas di <strong>{selectedTech.technician?.area_tugas || 'Belum Ditentukan'}</strong>, sedangkan lokasi pekerjaan client berada di <strong>{detectedJobArea}</strong>.
                                        </div>
                                    </div>
                                )}
                            ...
                            {/* Koordinator / Supervisor Lapangan (Opsional) */}
                            <div>
                                <div className="flex items-center justify-between">
                                    <Label htmlFor="supervisor_id" className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                                        <ShieldCheck className="w-3.5 h-3.5 text-slate-500" />
                                        <span>Koordinator / Supervisor Lapangan (Opsional)</span>
                                    </Label>
                                    <span className="text-[10px] text-slate-400">Verifikator / PIC</span>
                                </div>
                                <Select
                                ...
                                >
                                    <SelectTrigger className="mt-1.5 text-xs">
                                        <SelectValue placeholder="Pilih Penanggung Jawab Lapangan (Opsional)" />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <p className="text-[10px] text-slate-400 mt-1">
                                    * Untuk mengawasi jalannya pekerjaan tim atau memvalidasi checklist berita acara. Boleh dikosongkan jika teknisi bertugas mandiri.
                                </p>
                            ...
                            {/* Status */}
                            <div>
                                <Label htmlFor="status" className="text-xs font-semibold text-slate-700">Status Jadwal</Label>
                                <Select
                                    value={data.status}
                        ...
                        <div className="flex justify-end space-x-3 pt-5 border-t border-slate-200">
                            ...
                            >
                                {processing ? 'Menyimpan...' : 'Perbarui Jadwal'}
                            </Button>
                        </div>
                    </form>
                </div>
