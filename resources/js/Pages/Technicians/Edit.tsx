...
export default function Edit({ technician, users }: Props) {
    const [newSkill, setNewSkill] = useState('');

    const { data, setData, put, processing, errors } = useForm<FormData>({
        employee_id: technician.employee_id || '',
        user_id: technician.user_id ? String(technician.user_id) : '',
        nama: technician.nama || '',
        telepon: technician.telepon || '',
        email: technician.email || '',
        jabatan: technician.jabatan || 'Teknisi Field',
        status: technician.status || 'aktif',
        area_tugas: technician.area_tugas || '',
        keahlian: technician.keahlian || [],
        tanggal_bergabung: technician.tanggal_bergabung || '',
        foto_profil: technician.foto_profil || '',
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('technicians.update', technician.id));
    };

    return (
        <AppLayout>
            <Head title={`Edit Teknisi: ${technician.nama}`} />

            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-display-sm font-semibold text-ink">Edit Profil Teknisi: {technician.nama}</h1>
                        <p className="text-body-sm text-mute mt-1">Perbarui informasi profil dan penugasan teknisi.</p>
                    </div>
